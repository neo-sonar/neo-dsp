...
TEMPLATE_TEST_CASE("neo/fft/transform/radix2: test_path(c2c)", "", double)
{
    ...
    SECTION("plan")
    {
        auto test = [=](auto kernel) {
            ...
            auto order = neo::fft::ilog2(inout.size());
            auto plan  = neo::fft::fft_radix2_plan<Complex, decltype(kernel)>{order};
...
TEMPLATE_TEST_CASE("neo/fft/transform/radix2: roundtrip(fft_radix2_plan)", "", float, double)
{
    ...
    auto test = [=](auto kernel) {
        ...
        auto plan = neo::fft::fft_radix2_plan<std::complex<Float>, decltype(kernel)>{order};
